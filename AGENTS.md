...
This file defines the required implementation standards for contributors and coding agents working in this repository.
Treat these rules as project policy, not suggestions.
...
5. Response messages, exception messages, and other repeated application messages MUST be defined as constants instead
   of inline string literals.

## Frontend Contract Documentation

1. [FRONTEND_REFERENCE.md] MUST be updated whenever the backend contract changes.
2. Backend contract changes include any change to:
    - database schema
    - table fields
    - controllers
    - endpoints
    - request DTOs
    - response DTOs
    - repeated response or exception messages that frontend depends on
3. Contributors MUST keep `FRONTEND_REFERENCE.md` aligned with the current project state in the same change set as the
   backend change itself.
4. Do NOT leave `FRONTEND_REFERENCE.md` for a later cleanup when the change affects frontend integration or API
   understanding.
...
8. Repository method names MAY stay more descriptive when required by Spring Data query derivation or query intent, but
   service and controller methods MUST keep the standard CRUD naming style.
